...
%% ******************QPSK Modulation**************************************%
        ...
        for i=1:2:length(x_diff)-1
            if x_diff(i)==0 && x_diff(i+1)==0        % 3rd Quadrant
                g=cosd(225)+1j*sind(225);
            elseif x_diff(i)==0 && x_diff(i+1)==1    % 2nd Quadrant
                g=cosd(135)+1j*sind(135);
            elseif x_diff(i)==1 && x_diff(i+1)==0    % 4th Quadrant
                g=cosd(315)+1j*sind(315);
            elseif x_diff(i)==1 && x_diff(i+1)==1    % 1st Quadrant
...
%% ***************** Channel *************************************%    
        ...
        r=gg+sigma*noise;
%% ***************** QPSK Demodulation ***********************************%
        I=(real(r)>0); % Inphase component
        Q=(imag(r)>0); % Perpendicular component
        x_out=[];
        for q=1:length(r)
